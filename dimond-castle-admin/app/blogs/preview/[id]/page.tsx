...
import type { BlogInput, Block } from "../../../lib/blogs";
...
function renderBlock(block: Block, index: number) {
  switch (block.type) {
    case "heading":
      ...
      return (
        <Tag className="mt-8 font-semibold text-slate-900" key={`${block.type}-${index}`}>
          {block.text}
        </Tag>
      );
    case "paragraph":
      return (
        <p className="mt-4 text-slate-600" key={`${block.type}-${index}`}>
          {block.text}
        </p>
      );
    case "image":
      return (
        <figure className="mt-6" key={`${block.type}-${index}`}>
          <div className="relative h-[360px] w-full overflow-hidden rounded-3xl">
            <Image
              src={`https://res.cloudinary.com/demo/image/upload/w_900,f_auto/${block.publicId}.jpg`}
    ...
    case "quote":
      return (
        <blockquote
          ...
          className="mt-6 rounded-3xl border-l-4 border-emerald-500 bg-emerald-50/80 p-6 text-lg italic text-emerald-900"
        >
          {block.text}
...
export default async function BlogPreviewPage({ params, searchParams }: PreviewPageProps) {
  let content: BlogInput | null = null;

  if (searchParams.draft) {
    try {
      content = JSON.parse(decodeURIComponent(searchParams.draft)) as BlogInput;
    } catch (error) {
  ...
  return (
    <main className="mx-auto max-w-4xl space-y-6 px-4 py-16">
      <header className="space-y-4 text-center">
        <p className="text-sm uppercase tracking-[0.2em] text-emerald-500">Dimond Castle Blogs</p>
      ...
      <article className="space-y-4 text-left text-slate-700">
        {content.en.blocks.map((block, index) => renderBlock(block, index))}
      </article>
    </main>
  );
}
