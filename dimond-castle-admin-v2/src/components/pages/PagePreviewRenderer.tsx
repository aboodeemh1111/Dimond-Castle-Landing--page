...
import { type Page, type Section, type Row, type GridCol, type Block } from "@/lib/pages-api"
...
function getResponsiveClasses(value: any, map: Record<string, string>) {
  if (!value) return ""
...
function BlockRenderer({ block, locale }: { block: Block; locale: "en" | "ar" }) {
  ...
  switch (block.type) {
    case "heading": {
      ...
      if (block.level === 1) return <h1 className={headingClass}>{headingText}</h1>
      if (block.level === 2) return <h2 className={headingClass}>{headingText}</h2>
      if (block.level === 3) return <h3 className={headingClass}>{headingText}</h3>
    ...
    case "icon-feature":
      return (
        <div className="space-y-3">
          {block.icon && <div className="text-3xl text-green-600">{block.icon}</div>}
          <h3 className="text-xl font-semibold">{(block as any)[title]}</h3>
          {(block as any)[text] && <p className="text-slate-600">{(block as any)[text]}</p>}
        </div>
      )
...
export function PagePreviewRenderer({ page, locale }: Props) {
  const content = page[locale]

  if (!content.sections?.length) {
    return (
      <div className="rounded-md border border-dashed p-8 text-center text-muted-foreground">
        No sections for this locale yet.
      </div>
    )
  }
