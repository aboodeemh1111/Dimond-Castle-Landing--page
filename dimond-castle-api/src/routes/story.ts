...
import { storySettingsSchema } from '../validation/story'
...
const defaultStory = {
  ...
  introEN:
    'We are a company established in 2015 in the rice sector within the Kingdom of Saudi Arabia, driven by a clear vision to provide high-quality rice products that meet the needs of the local market and exceed consumer expectations.',
  introAR:
    'نحن شركة تأسست عام 2015 في قطاع الأرز داخل المملكة العربية السعودية، بدافع رؤية واضحة لتقديم منتجات أرز عالية الجودة تلبي احتياجات السوق المحلي وتتجاوز توقعات المستهلكين.',
  bulletsEN: [
...
router.get('/settings', async (_req, res, next) => {
  try {
    const settings = await StorySettings.findOne().sort({ updatedAt: -1 }).lean()
    if (!settings) return res.json(defaultStory)
...
router.put('/settings', async (req, res, next) => {
  try {
    const parsed = storySettingsSchema.safeParse(req.body)
    if (!parsed.success) {
      return res.status(400).json({ error: parsed.error.flatten() })
    }
    const payload = parsed.data
    const saved = await StorySettings.findOneAndUpdate({}, payload, { new: true, upsert: true, setDefaultsOnInsert: true })
