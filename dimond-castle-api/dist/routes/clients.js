...
const ClientSettings_1 = __importDefault(require("../models/ClientSettings"));
const clients_1 = require("../validation/clients");
...
router.get('/settings', async (_req, res, next) => {
    try {
        const settings = (await ClientSettings_1.default.findOne().lean());
...
router.put('/settings', async (req, res, next) => {
    try {
        const parsed = clients_1.clientSettingsSchema.safeParse(req.body);
        if (!parsed.success) {
        ...
        const normalizedClients = payload.clients
            .map((client, index) => {
            const clean = { ...client };
            if (clean._id && !mongoose_1.Types.ObjectId.isValid(clean._id)) {
                delete clean._id;
            }
            return {
                ...clean,
        ...
        const saved = await ClientSettings_1.default.findOneAndUpdate({}, { clients: normalizedClients, updatedBy: payload.updatedBy, enabled: payload.enabled ?? true }, {
            new: true,
            upsert: true,
