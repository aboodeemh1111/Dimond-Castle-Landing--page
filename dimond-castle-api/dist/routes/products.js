...
const products_1 = require("../validation/products");
const router = (0, express_1.Router)();
// ===== PUBLIC ROUTES =====
// Get all published products (for website)
router.get('/public', async (req, res) => {
    try {
        const query = products_1.productQuerySchema.parse({
            ...req.query,
            status: 'published',
...
router.get('/public/:slug', async (req, res) => {
    try {
        ...
        await Product_1.default.updateOne({ _id: product._id }, { $inc: { viewCount: 1 } });
        res.json(product);
    }
    catch (error) {
...
router.get('/:id', async (req, res) => {
    try {
        const product = await Product_1.default.findById(req.params.id).select('-__v').lean();
        if (!product) {
            return res.status(404).json({ error: 'Product not found' });
        }
        res.json(product);
...
router.post('/', async (req, res) => {
    try {
        const data = products_1.createProductSchema.parse(req.body);
        // Check if slug already exists
        const existing = await Product_1.default.findOne({ slug: data.slug });
        if (existing) {
...
router.put('/:id', async (req, res) => {
    try {
        const data = products_1.updateProductSchema.parse(req.body);
        ...
        const product = await Product_1.default.findByIdAndUpdate(req.params.id, { $set: data }, { new: true, runValidators: true }).select('-__v');
        if (!product) {
            return res.status(404).json({ error: 'Product not found' });
        }
        res.json(product);
...
router.post('/bulk/reorder', async (req, res) => {
    try {
        ...
        const updates = products.map((item, index) => Product_1.default.updateOne({ _id: item.id }, { $set: { order: index } }));
